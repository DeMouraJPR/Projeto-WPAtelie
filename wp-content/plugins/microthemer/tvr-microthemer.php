		...
		class tvr_microthemer_admin {
			...
			function __construct(){
				...
				if ( is_admin() and in_array($page, $this->all_pages) ) {
					...
					// delete debug data stored in session
					if (isset($_GET['clear_msg_in_session'])){

					    $this->clear_msg_in_session();
                    }
			...
			function maybe_manually_override_preferences(){
				...
				/*$this->cache_msg_in_session(true, array(array(
					'short' => 'hover_inspect_off_initially setting checked and hover_inspect set: ' . $this->preferences['hover_inspect'],
					'long' => 'Called by: ' . debug_backtrace()[1]['function'],
					'type' => 'notice'
				)));*/
			}
...
<p>JSON Error code: '. $this->json_last_error() . '</p>';
			...
			function cache_msg_in_session($append = false, $new_message = false){

				if (session_status() === PHP_SESSION_NONE) {
					session_start();
				}

			    $new_message = $new_message ? $new_message : $this->globalmessage;

				if ($append && !empty($_SESSION['cached_mt_messages'])){
					$new_message = array_merge(
						$_SESSION['cached_mt_messages'],
						$new_message
					);
				}

				$_SESSION['cached_mt_messages'] = $new_message;
			}

			function clear_msg_in_session(){

				if (session_status() === PHP_SESSION_NONE) {
					session_start();
				}

			    unset($_SESSION['cached_mt_messages']);
			}
			...
			function display_log(){
				...
				if (TVR_DEV_MODE && !empty($this->globalmessage) && isset($_SESSION['cached_mt_messages'])){
					$this->globalmessage = array_merge($this->globalmessage, $_SESSION['cached_mt_messages']);
				}
	...
	if (!class_exists('tvr_microthemer_frontend')) {
		...
		class tvr_microthemer_frontend {
			...
			*/
			...
			function get_current_post_data($id = 0){
